...
import {
  addQuoteItemModalAction,
  createQuoteDraftModalAction,
...
export function FloatingQuoteButton({
  ...
  function openAndCreateQuote() {
    setIsOpen(true);
    setQuoteState(initialQuoteState);
    startCreating(async () => {
      const result = await createQuoteDraftModalAction();
  ...
  return (
    <>
      <Button disabled={isCreating} onClick={openAndCreateQuote} type="button">
        <Plus aria-hidden="true" />
        {isCreating ? "Creando" : "Nueva cotizacion"}
      </Button>
...
function QuoteModal({
  ...
  isCreating: boolean;
  onClose: () => void;
  quoteState: CreateQuoteModalState;
}) {
  ...
  const [isAddingItem, startAddingItem] = useTransition();
  ...
  const effectiveCustomerName =
    selectedCustomer?.nombre ?? localCustomerMatch?.nombre ?? haciendaName ?? "Sin cliente";
  ...
  const quoteReady = quoteState.status === "success" && Boolean(quoteState.cotizacionId);
  ...
  function addItem(event: FormEvent<HTMLFormElement>) {
    ...
    if (!quoteState.cotizacionId) {
      setItemMessage("Primero debe crearse la cotizacion.");
      return;
    }
    ...
    startAddingItem(async () => {
      const result = await addQuoteItemModalAction(formData);
      setItemMessage(result.message);

      if (result.status === "success" && result.item) {
        setItems((currentItems) => [...currentItems, result.item as LocalQuoteItem]);
  ...
  return (
    <div
      ...
      role="dialog"
    >
      <div className="absolute inset-y-0 left-0 right-0 flex items-center justify-center overflow-auto p-4 lg:left-[280px]">
        <div className="grid h-[min(710px,calc(100vh-3rem))] w-[min(1200px,calc(100vw-2rem))] grid-cols-[1.08fr_0.92fr] gap-3 rounded-[1.4rem] border border-white/15 bg-[#eef1f4] p-3 shadow-[0_28px_80px_rgba(0,0,0,0.42)]">
          <section className="min-h-0 overflow-y-auto rounded-2xl bg-background p-4 shadow-sm ring-1 ring-black/5">
            <div className="mb-3 flex items-start justify-between gap-4">
              <div>
                ...
                <h2
                  ...
                  id="new-quote-title"
                >
                  {quoteState.numero ?? "Nueva cotizacion"}
                </h2>
              </div>
              <button
            ...
            <div className="mb-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-emerald-950">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-black">
                    {isCreating
                      ? "Creando cotizacion..."
                      : quoteState.numero ?? "Preparando cotizacion"}
                  </p>
                  <p className="text-xs">
                    {quoteState.message ?? "El borrador se crea al abrir este popup."}
                  </p>
                </div>
              </div>
            </div>
                ...
                {selectedCustomer
                  ...
                  : localCustomerMatch
                    ...
                    : searchState.result?.source === "hacienda"
                      ...
                      : searchState.message ?? "Busca primero en CRM; si es cedula valida, consulta Hacienda."}
              </p>
            </section>
            ...
            <section className="rounded-xl border border-emerald-300 bg-emerald-50/50 p-3">
              ...
              <form className="space-y-3" onSubmit={addItem}>
                ...
                <div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                  ...
                  <Button
                    className="self-end"
                    disabled={isAddingItem || !quoteReady}
                    type="submit"
                  >
                    <Plus aria-hidden="true" />
              ...
              <div className="mt-3 max-h-48 space-y-2 overflow-y-auto">
                {items.length > 0 ? (
                  ...
                  <p className="rounded-lg border border-dashed bg-white p-3 text-sm text-muted-foreground">
                    Agrega productos o servicios para alimentar la proforma.
                  </p>
                )}
              </div>
            </section>
          ...
          <aside className="min-h-0 overflow-y-auto rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/5">
            <div className="min-h-full rounded-xl border bg-white p-6 shadow-sm">
              <div className="border-b pb-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 text-3xl font-black">
                      {quoteState.numero ?? "COTIZACION"}
                    </h3>
                    <p className="mt-1 text-sm text-slate-500">Documento comercial</p>
                  </div>
                ...
                <div className="mt-5 grid gap-4 rounded-lg bg-slate-50 p-4 text-sm md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="font-bold">Borrador</p>
              ...
              <div className="mt-6 rounded-lg border border-dashed p-4 text-xs text-slate-500">
                Vista previa renderizada dentro del popup. PDF y envio formal se
                conectaran en una fase posterior.
              </div>
            </div>
          </aside>
        </div>
