...
function messageLabel(message: InboxMessage, channel: InboxChannel) {
  if (message.esNotaInterna || message.direccion === "interna") return "Nota interna";
  const channelLabel = CHANNEL_LABELS[channel] ?? "Inbox";
  if (message.direccion === "saliente") return `Saliente ${channelLabel}`;
  return `Entrante ${channelLabel}`;
}

export function WhappMessageThread({ channel, messages }: WhappMessageThreadProps) {
  return (
          ...
          return (
            ...
            >
              <div className="mb-1 flex items-center justify-between gap-3 text-xs opacity-75">
                <span>{messageLabel(message, channel)}</span>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-xs opacity-75">
                ...
                {message.canalMessageId ? (
                  <span className="break-all font-mono">
                    ID Meta: {message.canalMessageId}
                  </span>
                ) : null}
