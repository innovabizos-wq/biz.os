...
import {
  listBrainWorkflowCatalog,
  prepareBrainWorkflow,
} from "@/modules/brain/workflow-service";
...
export async function GET() {
  ...
  return NextResponse.json(listBrainWorkflowCatalog(context.tenant));
}
...
export async function POST(request: Request) {
  ...
  const result = await prepareBrainWorkflow(context.tenant, payload);
  if (!result.ok) {
