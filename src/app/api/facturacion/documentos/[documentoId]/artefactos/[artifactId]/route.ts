...
import { getFiscalDocumentArtifactForDownload } from "@/modules/billing/queries";
import { requireAdminAccess } from "@/modules/tenant/admin-access";

type FiscalArtifactDownloadRouteProps = {
  params: Promise<{ artifactId: string; documentoId: string }>;
};
...
export async function GET(_request: Request, { params }: FiscalArtifactDownloadRouteProps) {
  const [{ artifactId, documentoId }, access] = await Promise.all([
    params,
    requireAdminAccess(),
  ]);
  ...
  const artifact = await getFiscalDocumentArtifactForDownload(
    access.tenant,
    documentoId,
    artifactId,
  );
